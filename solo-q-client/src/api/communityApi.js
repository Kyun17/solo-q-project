...
export async function createPost(payload) {
    const res = await api.post('/api/community', payload);
    return res.data; // PostDetailResponse
}

// 게시글 수정 (회원만)
export async function updatePost(postId, payload) {
    const res = await api.patch(`/api/community/${postId}`, payload);
    return res.data; // PostDetailResponse
}
...
export async function createComment(postId, payload) {
    const res = await api.post(`/api/community/${postId}/comments`, payload);
    return res.data; // CommentResponse
}

// 댓글 수정 (회원만)
export async function updateComment(commentId, payload) {
    const res = await api.patch(`/api/comments/${commentId}`, payload);
    return res.data; // CommentResponse
}
