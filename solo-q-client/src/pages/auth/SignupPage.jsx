...
import { ArrowLeft, Mail, Lock, User, CheckCircle, ArrowRight, Loader2 } from 'lucide-react';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const [response] = await Promise.all([
                axios.post('http://localhost:8080/api/auth/signup', requestBody),
        ...
        <div className="min-h-screen flex flex-col justify-center items-center px-4 font-sans text-white relative overflow-hidden bg-[#0a0a1a]">
            ...
            <div className="w-full max-w-md flex flex-col items-center relative z-10">
                ...
                <Link to="/" className="flex items-center gap-3 mb-10 group cursor-pointer">
                    ...
                    <span className="font-bold text-4xl tracking-tight drop-shadow-lg">Solo<span className="text-purple-400">-Q</span></span>
                </Link>
                ...
                <div className="w-full bg-slate-900/60 backdrop-blur-md p-1.5 rounded-xl flex mb-8 border border-slate-700/50">
                    <Link to="/auth/login" className="flex-1 py-3 rounded-lg text-slate-400 font-medium hover:text-white transition-all text-center hover:bg-slate-800/50">
                        로그인
                    </Link>
                ...
                <form onSubmit={handleSubmit} className="w-full space-y-5">
                    ...
                    {/* 닉네임 */}
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2 ml-1">이름 (닉네임)</label>
                    ...
                    {/* 이메일 */}
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2 ml-1">이메일</label>
                        <div className="relative group">
                            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-purple-400 transition-colors">
                                <Mail size={20} />
                            </div>
                    ...
                    {/* 비밀번호 */}
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2 ml-1">비밀번호</label>
                        <div className="relative group">
                            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-purple-400 transition-colors">
                                <Lock size={20} />
                            </div>
                    ...
                    {/* 비밀번호 확인 */}
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2 ml-1">비밀번호 확인</label>
                        <div className="relative group">
                            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-purple-400 transition-colors">
                                <CheckCircle size={20} />
                            </div>
