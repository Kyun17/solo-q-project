...
import { Video, Mic, CheckCircle, PlayCircle, ArrowLeft, AlertCircle } from 'lucide-react';
            ...
            <div className="max-w-7xl mx-auto px-6 pt-32 pb-20">
                <div className="mb-10">
                    ...
                    <p className="text-slate-400 mt-2">
                        실전 면접을 시작하기 전, 카메라와 오디오 상태를 확인하고 옵션을 설정하세요.
                    </p>
                </div>
                ...
                <div className="grid lg:grid-cols-2 gap-10">
                    ...
                    <div className="relative aspect-video bg-black rounded-3xl overflow-hidden border border-slate-700 shadow-2xl group">
                        {permission === 'denied' ? (
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900 text-slate-400">
                                ...
                                <p className="text-sm mt-2 text-center">브라우저 주소창의 자물쇠 아이콘을 눌러<br />권한을 허용해주세요.</p>
                            </div>
                        ) : (
                            <>
                                <Webcam
                                ...
                                {permission === 'granted' && (
                                    <div className="absolute bottom-6 left-6 bg-black/60 backdrop-blur px-4 py-2 rounded-full flex items-center gap-2 border border-white/10">
                                        ...
                                        <span className="text-sm font-bold text-green-400">Camera Active</span>
                                    </div>
                                )}
                            </>
                        )}
                    ...
                    <div className="flex flex-col justify-center space-y-6">
                        <div className="bg-slate-800/40 border border-white/5 p-6 rounded-3xl space-y-6 backdrop-blur-sm">
                            <h3 className="text-xl font-bold border-b border-white/5 pb-4">면접 옵션 설정</h3>

                            {/* 카테고리 선택 */}
                            <div>
                                <label className="block text-slate-400 mb-2 text-sm font-medium">질문 카테고리</label>
                                <select
                                    value={category}
                            ...
                            {/* 문제 수 선택 */}
                            <div>
                                <label className="block text-slate-400 mb-2 text-sm font-medium">진행할 문제 수</label>
                                <select
                                    value={questionCount}
                        ...
                        <button
                            ...
                            className={`w-full py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-3 transition-all transform hover:scale-[1.02] active:scale-95 shadow-xl ${permission === 'granted'
