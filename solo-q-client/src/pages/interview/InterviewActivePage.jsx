    ...
    const handleFinishInterview = async () => {
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                navigate('/interview/finished', {
                    state: {
                        result: { totalSeconds: totalElapsedTime, questionCount: questions.length },
        ...
        <div className="relative h-[100dvh] w-full bg-black overflow-hidden">
            ...
            <div className="absolute top-0 left-0 right-0 z-50 p-6 flex justify-between items-start pointer-events-none">
                <div className="bg-black/40 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-full flex items-center gap-2 pointer-events-auto">
                    <div className="relative w-3 h-3">
                        <div className={`absolute top-0 left-0 w-full h-full bg-red-500 rounded-full ${isRecording ? 'animate-ping opacity-75' : ''}`}></div>
                        <div className={`relative w-3 h-3 rounded-full ${isRecording ? 'bg-red-500' : 'bg-gray-400'}`}></div>
                    </div>
                    ...
                    <div className="flex items-start gap-6">
                        ...
                        <div className="relative w-14 h-14 shrink-0 flex items-center justify-center">
                            <svg className="w-full h-full transform -rotate-90">
                                <circle cx="28" cy="28" r="22" stroke="currentColor" strokeWidth="4" fill="transparent" className="text-slate-700/50" />
                                <circle
                                    cx="28" cy="28" r="22"
                                    stroke="currentColor" strokeWidth="4" fill="transparent"
                                    className={`transition-all duration-1000 ease-linear ${isUrgent ? 'text-red-500' : 'text-purple-400'}`}
                                    strokeDasharray={2 * Math.PI * 22}
                                    strokeDashoffset={(2 * Math.PI * 22) - (timeLeft / TIME_LIMIT) * (2 * Math.PI * 22)}
                                    strokeLinecap="round"
                                />
                            </svg>
                            <span className={`absolute text-lg font-bold ${isUrgent ? 'text-red-400 animate-pulse' : 'text-white'}`}>
                                {timeLeft}
                            </span>
                        </div>
                    </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-black via-black/90 to-transparent z-50 flex items-center justify-center gap-6 px-6">
                <button
                    ...
                    title="면접 중단"
                >
                    <StopCircle size={24} weight="fill" className="group-hover:scale-110 transition-transform" />
                </button>
